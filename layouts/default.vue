<template>
  <section id="informazioni_personali">
    <h1>{{ $t('anagrafica.title') }}</h1>
    <dl>
        <dt>
            {{ $t('anagrafica.nominativo.title') }}
        </dt>
        <dd>
            <p>
                <span id="nome">{{ $t('anagrafica.nominativo.nome') }}</span> <span id="cognome">{{ $t('anagrafica.nominativo.cognome') }}</span>
            </p>
        </dd>
        <!-- @if($withMail)
        <dt>Email</dt>
